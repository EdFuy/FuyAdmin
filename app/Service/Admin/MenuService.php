...
class MenuService extends BaseService
{
	...
	public function __construct(MenuRepositoryEloquent $menu,Route $route)
	{
		$this->menu = $menu;
		$this->route = $route;
	}
...
//        return $this->sortMenuSetCache();
    ...
    public function getMenuList1()
    {
        ...
        $menuListArray = [];
        if($menu->url_id_path){
            $head_url_id = explode(',',$menu->url_id_path)[0];
            $route_name = DB::table('Menus')->where('id',$menu->id)->first()->route_name;
        }else{
            $head_url_id = $menu->id;
            $route_name = $menu->route_name;
        }
        $menuList = DB::table('Menus')->where('pid',$head_url_id)->get();
        foreach($menuList as &$menulist){
            ($menulist->route_name == $menu->route_name) ? ($menulist->is_active = 1) : ($menulist->is_active = 0);
            $menuListArray[$menulist->grp][] = $menulist;
            if($menulist->route_name == $menu->route_name){
                $menuListArray[$menulist->grp]['expand'] = 'expand';
            }

        }
//        dump($menuListArray);die;
...
//        dump($menuList);die;
    ...
    public function getHeadUrl(){
        ...
        if($menu->url_id_path){
            $head_url_id = explode(',',$menu->url_id_path)[0];
            $header_route_name = DB::table('Menus')->where('id',$head_url_id)->first()->route_name;
        }else{
            $header_route_name = $menu->route_name;
        }
        return $header_route_name;

    }
    public function getMenu(){
        $current_route_name = Route::currentRouteName();
        $menu = DB::table('Menus')->where('route_name',$current_route_name)->first();
        return $menu;
    }
}