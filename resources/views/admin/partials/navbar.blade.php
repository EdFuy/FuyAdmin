...
{{--        {!!$menuPresenter->sidebarMenuList($sidebarMenu)!!}--}}
        ...
        <ul class="nav">
            @foreach ($sidebarMenu as $kmenu=>$vmenu)
                <li class="has-sub {{isset($vmenu['expand']) ? $vmenu['expand']:''}}">
                    <a href="#">
                        <b class="caret pull-right"></b>
                        <i class="fa fa-suitcase"></i>
                        <span>{{$kmenu}}</span>
                    </a>
                    <ul class="sub-menu" style=" {{isset($vmenu['expand']) ? 'display:block': 'display:none'}}">
                        @foreach($vmenu as $v)
                            @if (is_object($v))
                            <li {{$v->is_active ? "class=active":"class=''"}}><a href="{{route($v->route_name)}}">{{$v->title}}</a></li>
                            @endif
                         @endforeach
                    </ul>
                </li>
